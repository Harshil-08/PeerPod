      ...
      <nav className="fixed top-0 z-50 w-full bg-white border-b border-gray-200">
        <div className="px-3 py-3 lg:px-5 lg:pl-3">
          <div className="flex items-center justify-between">
			...
			<aside
				id="logo-sidebar"
				className={`fixed shadow-xl top-0 left-0 z-40 w-64 h-screen pt-20 transition-all duration-300 ${IsOpen ? "-translate-x-0" : "-translate-x-full"
			...
			{allRooms.map((room) => {
				...
				return (
					<li key={room.id}>
						<Link
							to={`/chat/${room.id}`}
							className={`flex justify-between items-center p-2 text-gray-900 rounded-lg ${isActive ? "bg-gray-100" : ""
	...
	return (
		<li>
			<Link
				...
				className={`flex justify-start items-center p-2 text-gray-900 border border-gray-500/50 rounded-lg ${isActive ? "bg-gray-100" : ""
					} dark:hover:bg-gray-100 group`}
			>
				<img
